...
def guesser():
    ...
    william = pd.DataFrame(columns = ['Ticker', 'Yesterdays Predicted Close', 'Yesterdays Close', 'Todays Predicted Close', 'Todays Actual Close', 'Difference'])
    for i in tickerlist:
        ticker = tickerlist[will].upper()
        stock = yf.Ticker(ticker)
        stock_hist = stock.history(period=period, interval="1d")
    ...
    william.to_csv(filepath + str(now_) + ' Daily ETF Prediciton' + '.csv', index=False)
    global guessertime
