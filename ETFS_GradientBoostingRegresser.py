...
def guesser():
    ...
    william = pd.DataFrame(columns = ['Ticker', 'Yesterdays Close', 'Todays Predicted Close', 'Todays Actual Close', 'Difference', 'Tomorrows Predicted Close'])
    for i in tickerlist:
        ticker = tickerlist[will].upper()
        stock = yf.Ticker(ticker)
        stock_hist = stock.history(period=period, interval="1d")
    ...
    #open model
        ...
        william = william.append({'Ticker' : ticker, 'Yesterdays Close' : float(yest_close_fixed), 'Todays Predicted Close' : float(y_pred_fixed), 'Todays Actual Close' : float(elizabeth2.tail(1)["Actual_Close"]), 'Difference' : float(elizabeth2.tail(1)["Actual_Close"]) - float(y_pred_fixed), 'Tomorrows Predicted Close' : float(y_pred_tmrw)}, ignore_index = True)
    ...
    william.to_csv(filepath + str(now_) + '_25_loser' + '.csv', index=False)
    global guessertime
